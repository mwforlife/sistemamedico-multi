...
if (isset($_POST['id'])) {
	...
	if ($object != null) {
        ...
        echo "<select class='form-control select2' id='provinciaedit' name='provinciaedit'>";
        $provincias = $c->listarprovincia($object->getRegion());
        if(count($provincias)>0){
            foreach ($provincias as $p) {
                if ($p->getId() == $object->getProvincia()) {
            ...
            echo "<option value='0'>No hay provincias Registradas</option>";
        }
        echo "</select>";
        echo "</div>";
        echo "</div>";
		...
		echo "<button type='button' class='btn btn-primary' onclick='actualizarComunas(" . $object->getId() . ")'><i class='fas fa-save'></i> Guardar Cambios</button>";
