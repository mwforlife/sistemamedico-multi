...
if (isset($_POST['id'])) {
	...
	if ($object != null) {
		...
		echo "<button type='button' class='btn btn-primary' onclick='actualizarEcog(" . $object->getId() . ")'><i class='fas fa-save'></i> Guardar Cambios</button>";
		echo "<button type='button' class='btn btn-danger' data-dismiss='modal'><i class='fas fa-times'></i> Cancelar</button>";
		echo "</div>";
		echo "</div>";
	} else {
		echo "<div class='alert alert-danger' role='alert'>No se encontró la Ecog</div>";
	}
}
