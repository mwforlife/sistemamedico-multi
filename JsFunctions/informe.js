...
function validartnm(t1,t2,t,ttext,n1,n,ntext,m1,m,mtext,m2){
    for(var i=0; i<tnm.length; i++){
        if(tnm[i].t1==t1 && tnm[i].t2==t2 && tnm[i].ttext==ttext && tnm[i].n1==n1 && tnm[i].n==n && tnm[i].ntext==ntext && tnm[i].m1==m1 && tnm[i].m==m && tnm[i].mtext==mtext && tnm[i].m2==m2){
            return true;
        }
    }
    return false;
...
function vistapreviainforme(paciente, comite){
    ...
    $("#frameprevia").attr("src", "php/reporte/previa/informecomite.php?informe="+JSON.stringify(informe));
...
function cargartnm(){
    ...
    for(var i=0; i<tnm.length; i++){
        ...
        html += "<td>"+tnm[i].m2+"</td>";
        html += "<td><button type='button' class='btn btn-danger' onclick='eliminartnm("+i+")'><i class='fas fa-trash-alt'></i></button></td>";
        html += "</tr>";
    }
    $("#tnmbody").html(html);
}
